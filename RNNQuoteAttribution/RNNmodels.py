...
class Config(object):
    ...
    # wordvecpath = "glove.6B.300d.word2vec.txt"
    # wordvecpath = "glove.6B.100d.filtered.txt"
    wordvecpath = "glove.6B.100d.word2vec.txt"
...
class WhoseLineModel(object):
    ...
    def load_data(self, debug=False):
        ...
        self.wordvecs = gensim.models.KeyedVectors.load_word2vec_format(self.config.wordvecpath, binary=False)
        self.vocab = Vocab()
...
class WhoseLineSoftmaxModel(WhoseLineModel):
    ...
    def add_loss_op(self, y):
        loss = tf.reduce_mean(tf.multiply(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=y), self.loss_weights_placeholder))
        weight_matrices = [v for v in tf.global_variables() if "weights" in v.name or "Matrix" in v.name]
...
def teste(sentencernn, contextrnn, sentencebi, contextbi, phrases):
    config = Config()
    config.bidirectional_sentences = sentencebi
    config.bidirectional_conversations = contextbi
    config.batch_size = len(phrases)
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            ...
            for phrase in phrases:
                ch, speaker, line = phrase.split("\t")
                encoded_line = (np.array([model.vocab.encode(word) for word in line.split()], dtype=np.int32),
                            model.speakers.encode(speaker))
                dataset.append(encoded_line)
                print phrase

            print '=-=-='
            test_loss, predictions = model.predict(session, dataset)
            # test_loss, predictions = model.predict(session, model.test_data)

            if model.config.batch_size == "chapter":
                test_ground_truth = [dp[1] for chapter in model.test_data for dp in chapter]
            else:
                test_ground_truth = [dp[1] for dp in dataset]

            print 'predictions:'
            print predictions
            decodePred = []
            for p in predictions:
                decodePred.append(str(model.speakers.decode(p)))
            print decodePred
            print 'truth:'
            decodeTruth = []
            for t in test_ground_truth:
                decodeTruth.append(str(model.speakers.decode(t)))
            print decodeTruth
            # confusion2 = confusion_matrix(test_ground_truth, predictions)
            # print 'confusion2:'
            # print confusion2
